<template>
    <ul class="pagination">
        <li :class="[currentPage < 2 && 'pagination_arrow-disable']" class="pagination_arrow pagination_arrow-prev"
            @click="() => onClick(currentPage - 1)">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="pagination_arrow-icon">
        ...
        <li v-if="props.totalItems > props.pageSize" v-for="page in totalPagesShow.slice(start, end)"
            class="pagination_item" :class="{ active: page === currentPage }" :key="page" @click="() => onClick(page)">
            {{ page }}
        </li>
        <li :class="[currentPage >= pageCount && 'pagination_arrow-disable']" class="pagination_arrow pagination_arrow-prev"
            @click="() => onClick(currentPage + 1)">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="pagination_arrow-icon">
                <path fillRule="evenodd"
